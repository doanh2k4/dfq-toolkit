...
from models.qatops import LSQconv2d
from models.ptqops import Qconv2d
from models.lsqops import QuantConv2d
...
class Detect(nn.Module):
    ...
    def __init__(
        self, nc=80, anchors=(), ch=(), inplace=True, num_bits=8, mode="exact"
    ):
        ...
        # self.m = nn.ModuleList(LSQconv2d(x, self.no * self.na, 1, num_bits=num_bits, mode=mode) for x in ch)  # output conv
        # self.m = nn.ModuleList(LSQConv(x, self.no * self.na, 1, num_bits=num_bits, mode=mode) for x in ch)  # output conv
        # self.m = nn.ModuleList(QuantConv2d(x, self.no * self.na, 1, a_bits=num_bits, w_bits=num_bits) for x in ch)  # output conv
        # self.m = nn.ModuleList(WandaConv2d(x, self.no * self.na, 1, sparsity_ratio=0.1) for x in ch)  # output conv
        # self.m = nn.ModuleList(Qconv2d(x, self.no * self.na, 1, num_bits=num_bits, mode=mode) for x in ch)  # output conv
        self.inplace = inplace  # use inplace ops (e.g. slice assignment)
        self.drop = None
        drop_list = ["droptotal", "dropdetect"]
    ...
    def start_quantize(self):
        for conv in self.m:
            if not (type(conv) == nn.Conv2d):
                conv.start_quantize()

    def start_prune(self, sparsity=None, prune_mode=None):
        for conv in self.m:
            conv.start_prune(sparsity, prune_mode)
...
class BaseModel(nn.Module):
    ...
    def start_quantize(self):
        for m in self.model:
            if isinstance(m, (Conv, C3, SPPF, Detect)):
                m.start_quantize()

    def start_prune(self, sparsity=None, prune_mode=None):
        for m in self.model:
            if isinstance(m, (Conv, C3, SPPF, Detect)):
                m.start_prune(sparsity, prune_mode)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov5s.yaml", ch=3, nc=None, anchors=None, mode="exact"):
        ...
        self.model, self.save = parse_model(
            deepcopy(self.yaml), ch=[ch], mode=mode
        )  # model, savelist
        # print('*'*50)
...
def parse_model(d, ch, mode="exact"):
    ...
    for i, (f, n, bits, m, args) in enumerate(
        d["backbone"] + d["head"]
        ...
        if m in {Conv, C3, SPPF, Detect}:
            m_ = (
                nn.Sequential(*(m(*args, num_bits=bits, mode=mode) for _ in range(n)))
                if n > 1
                else m(*args, num_bits=bits, mode=mode)
            )  # module
        else:
            m_ = (
...
if __name__ == "__main__":
    ...
    model = Model(opt.cfg, mode=opt.mode).to(device)
