...
# @threaded
def plot_boxes(images, targets, paths=None, fname="images.jpg", names=None):
    """Plots an image grid with labels from YOLOv5 predictions or targets, saving to `fname`."""
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    box_scale = min(w / box_w, h / box_h)
                    # if w/box_w < h/box_h:
                    #     tmp_h = (h/box_scale - box_h) / 2
                    #     box[1] -= tmp_h
                    #     box[3] += tmp_h
                    # else:
                    #     tmp_w = (w/box_scale - box_w) / 2
                    #     box[0] -= tmp_w
                    #     box[2] += tmp_w
                    extarct = annotator.im.crop(box)
                    extarct = extarct.resize(
                        (round(box_w * box_scale), round(box_h * box_scale))
                    )
                    if w / box_w < h / box_h:
                        tmp_h = round((h - box_h * box_scale) / 2)
                        extarct = ImageOps.expand(
                            extarct, border=(0, tmp_h), fill="black"
                        )
                    else:
                        tmp_w = round((w - box_w * box_scale) / 2)
                        extarct = ImageOps.expand(
                            extarct, border=(tmp_w, 0), fill="black"
                        )
                    extarct.save(os.path.join(dirname, f"extract_{len(results)-1}.png"))
            for j, box in enumerate(boxes.T.tolist()):
                cls = classes[j]
                color = colors(cls)
                cls = names[cls] if names else cls
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    label = f"{cls}" if labels else f"{cls} {conf[j]:.1f}"
                    annotator.box_label(box, label, color=color)
    with open(os.path.join(dirname, "extract.json"), "w") as fout:
